// Helps encrypt+decrypt from the AD save format.
// This is adapted from the AD source code.

import pako from "pako";

type StringOrArray = string | Uint8Array;

type SerializeEncodeFunction = (
    x: StringOrArray,
    type?: string
) => StringOrArray;
...
interface SerializeStep {
    ...
    encode: SerializeEncodeFunction;
    decode: SerializeEncodeFunction;
}

// Interface defining the structure of the game save serializer.
interface IGameSaveSerializer {
    serialize: (save: any) => string;
    jsonConverter: (key: any, value: any) => any;
    deserialize: (data: string) => any;
    encoder: TextEncoder;
    decoder: TextDecoder;
    startingString: { [key: string]: string };
    endingString: { [key: string]: string };
    version: string;
    steps: SerializeStep[];
    getSteps: (type: string, version: string) => SerializeStep[];
    encodeText: (text: string, type: string) => string;
    decodeText: (text: string, type: string) => string;
}

export const GameSaveSerializer: IGameSaveSerializer = {
    serialize(save: any) {
        const json = JSON.stringify(save, this.jsonConverter);
        return this.encodeText(json, "savefile");
    },
    jsonConverter(key: any, value: any) {
        if (value === Infinity) {
            return "Infinity";
        }
    ...
    steps: [
        ...
        {
            encode: (x: string) => GameSaveSerializer.encoder.encode(x),
            decode: (x: Uint8Array) => GameSaveSerializer.decoder.decode(x),
        },
        // This step is  where the compression actually happens. The pako library works with unsigned 8-bit arrays.
        {
            encode: (x: string) => pako.deflate(x),
        ...
        // We need to do this outselves because GameSaveSerializer.decoder would give us unicode sometimes.
        {
            encode: (x: Uint8Array) =>
                Array.from(x)
                    ...
                    .join(""),
            decode: (x: string) =>
                Uint8Array.from(Array.from(x).map((i) => i.charCodeAt(0))),
        },
        ...
        // won't break this.
        { encode: (x: string) => btoa(x), decode: (x: string) => atob(x) },
        ...
        {
            encode: (x: string, type: string) =>
                x + GameSaveSerializer.endingString[type],
            decode: (x: string, type: string) =>
                x.slice(
                    0,
                    x.length - GameSaveSerializer.endingString[type].length
                ),
            condition: (version: string) => version >= "AAB",
        },
    ],
    getSteps(type: string, version: string): SerializeStep[] {
        ...
        return this.steps
            .filter((i: SerializeStep) => !i.condition || i.condition(version))
            .concat({
                encode: (x: string) =>
                    `${
                        GameSaveSerializer.startingString[type] +
                        GameSaveSerializer.version
                    }${x}`,
                decode: (x: string) =>
